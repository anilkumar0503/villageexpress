...
export default function SupportScreen() {
  ...
  const createTicket = async () => {
    ...
    try {
      ...
      Alert.alert('Success', 'Ticket created! We will respond shortly.');
      setShowCreate(false);
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View style={modal.container}>
            ...
            <ScrollView style={modal.body}>
              ...
              <TextInput style={modal.input} value={subject} onChangeText={setSubject} placeholder="Brief description of your issue" />
              <Text style={modal.label}>Description</Text>
              <TextInput style={[modal.input, modal.textArea]} value={description} onChangeText={setDescription} placeholder="Detailed description..." multiline numberOfLines={5} textAlignVertical="top" />
        ...
        {selectedTicket && (
          <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            <View style={modal.container}>
              ...
              <View style={modal.ticketInfo}>
                <Text style={modal.ticketSubject}>{selectedTicket.subject}</Text>
                <View style={[modal.statusBadge, { backgroundColor: STATUS_COLOR[selectedTicket.status] }]}>
                  <Text style={modal.statusText}>{selectedTicket.status.replace('_', ' ')}</Text>
                </View>
              </View>
              <FlatList
              ...
              {selectedTicket.status !== 'CLOSED' && (
                <View style={modal.replyBar}>
                  ...
                  <TouchableOpacity style={modal.sendBtn} onPress={sendReply}>
...
const styles = StyleSheet.create({
  ...
  ticketItem: { backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 10, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 4, elevation: 2 },
...
const modal = StyleSheet.create({
  ...
  ticketInfo: { flexDirection: 'row', padding: 16, alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: '#eee' },
  ticketSubject: { fontSize: 15, fontWeight: '600', color: '#333', flex: 1, marginRight: 10 },
  ...
  sendBtn: { backgroundColor: THEME, borderRadius: 20, paddingHorizontal: 16, paddingVertical: 10 },
