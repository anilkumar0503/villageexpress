...
const ACCESS_TOKEN_KEY = '@ve_captain_access_token';
const REFRESH_TOKEN_KEY = '@ve_captain_refresh_token';
const USER_KEY = '@ve_captain_user';

const captainTokenStorage = {
...
const linking = {
  prefixes: ['ve-captain://', 'https://villageexpress.in/app/captain'],
  config: {
    screens: {
      ...
      MainTabs: {
        screens: {
          HomeTab: 'home',
          AssignmentsTab: { screens: { MyBookings: 'assignments', BookingDetails: 'assignments/:bookingId', Segments: 'segments' } },
...
function PushNotificationManager() {
  ...
  useEffect(() => {
    ...
    const removeResp = notificationService.addResponseListener(r => {
      ...
      if (data?.segmentId)    navigate('Segments');
      if (data?.screen === 'earnings') navigate('EarningsTab');
    });
...
export default function App() {
  useEffect(() => {
    setApiTokenStorage(captainTokenStorage);
    setAuthTokenStorage(captainTokenStorage);
  }, []);
