...
export default function EditProfileScreen({ navigation }: any) {
  ...
  // Profile info visible to PM (read-only)
  const [shopLocation, setShopLocation] = useState<{ pointName?: string; village?: string; district?: string } | null>(null);
  ...
  useEffect(() => {
    profileApi.getProfile().then(res => {
      ...
      setPhone(d?.phone ?? '');
      setEmail(d?.email ?? '');
      setShopName(d?.pointManagerProfile?.shopName ?? '');
      setShopLocation(d?.pointManagerProfile?.shopLocation ?? null);
  ...
  const handleSave = async () => {
    ...
    if (!shopName.trim() || shopName.trim().length < 2) { Alert.alert('Error', 'Shop name must be at least 2 characters'); return; }
    if (changePassword) {
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={styles.root} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Section title="Shop Details">
          <Field label="Shop / Business Name" value={shopName} onChangeText={setShopName} autoCapitalize="words" />
          {shopLocation && (
            <View style={styles.locationCard}>
              <Text style={styles.locationLabel}>📍 Your Location (set at registration)</Text>
              <Text style={styles.locationValue}>
                {[shopLocation.pointName, shopLocation.village, shopLocation.district].filter(Boolean).join(', ')}
              </Text>
              <Text style={styles.locationHint}>Contact admin to change your location</Text>
            </View>
          )}
        </Section>
...
const styles = StyleSheet.create({
  ...
  locationCard: { backgroundColor: '#FFF8E1', borderRadius: 10, padding: 12, marginTop: 12, borderWidth: 1, borderColor: '#FFD54F' },
  locationLabel: { fontSize: 12, color: '#795548', fontWeight: '600', marginBottom: 4 },
  locationValue: { fontSize: 14, color: '#333', fontWeight: '500', marginBottom: 4 },
  locationHint: { fontSize: 11, color: '#999', fontStyle: 'italic' },
